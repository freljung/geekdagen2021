...
I .NET 5 och senare är de aktiverade som standard. För tidigare versioner så kan de aktiveras genom att sätta ```<EnableNETAnalyzers>``` till ```true```
i projekt-filen
...
## Konfigurera övergripande

Som standard är det få regler som är aktiverade för .NET Analyzers. För att ändra den generella nivån så kan ```<AnalysisLevel>``` läggas till i 
projektfilen. Nivån sätts kortfattat till ```latest```, ```preview``` eller .NET versionens nummer. Nivån kan även kombineras med ett läge som avgör 
hur hårt reglerna ska appliceras. Läget, som även kan sättas med ```<AnalysisMode>```. Värdet sätts till i stigande nivå till en av 
```None```, ```Default```, ```Minimum```, ```Recommended```, ```All```. 

Läs mer om AnalysysLevel på [docs.microsoft.com](https://docs.microsoft.com/en-us/dotnet/core/project-sdk/msbuild-props#analysislevel).
...
Lägg till ny .editorconfig som genereras från nuvarande kod.
...
.editorconfig med standardinställningar för .NET C# och VB
